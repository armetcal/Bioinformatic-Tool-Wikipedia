...
\*\*Note: MetaPhlAn is part of the HUMAnN package installed in the humann.Rmd
script. One or both scripts need to be updated to just use the HUMAnN
installation, creating an 'annotation' pipeline.
...
Canada's MetaPhlAn python wheel (similar to a module) into it. **If this step
has been done previously, it can be skipped.**

The MetaPhlAn python wheel is available on Compute Canada. First check what the
most up-to-date version of MetaPhlAn is by typing
...
--bowtie2out metaphlan_out/bt_out/{}.bowtie2.txt
date
```
...
Annotated read counts can be exported, added to a phyloseq object, and analyzed
in R.

If performing [functional
annotation](https://armetcal.github.io/Bioinformatic-Tool-Wikipedia/Annotation/humann.html)
of the reads, the taxonomy tables generated here can be used as input for the
HUMAnN pipeline, greatly reducing the run time.
