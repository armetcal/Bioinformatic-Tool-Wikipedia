...
downloaded as explained in the tutorial linked above.
...
# the sample names, and then read them through our bbduk.sh pipeline.
# {} acts as a placeholder for the sample names.
ls sequences/*_1.fastq.gz | sed 's/sequences\///; s/_1.fastq.gz//' | bbduk.sh \
in=sequences/{}_1.fastq.gz \
in2=sequences/{}_2.fastq.gz \
out=bbduk_out/{}_1.fastq.gz \
out2=bbduk_out/{}_2.fastq.gz \
...
for file in sequences/*_1.fastq.gz; do
  ...
  bbduk.sh \
    ...
    out=bbduk_out2/"${sample}_1.fastq.gz" \
    out2=bbduk_out2/"${sample}_2.fastq.gz" \
    stats=bbduk_out2/stats/"${sample}_stats.txt" \
    tpe tbo qtrim=rl trimq=25 maq=25 minlen=50 ref=adapters,phix \
    -Xmx45g
done
```
...
already_run=$(ls hocort_out/*_1.fastq.gz | sed 's|.*/||; s/_1.fastq.gz//')
# List sample IDs from the stats files
all_samples=$(ls bbduk_out/*_1.fastq.gz | sed 's|.*/||; s/_1.fastq.gz//')
# Compare the lists to find sample IDs in FASTQ but not in stats
comm -23 <(echo "$all_samples" | sort) <(echo "$already_run" | sort) > missing_hocort_samples.txt
```
...
while read -r sample; do
  hocort map bowtie2 \
    ...
    --filter true
    --threads=37
done < missing_hocort_samples.txt
```
...
# Step 5: Remove Low-Quality Reads

While Step 3 removed reads that matched non-target DNA (adapters, etc), this
code removes reads that are low (quality (short, inaccurate base calls, etc).
This could theoretically be combined with Step 3, but are kept separate here
because Step 3 is already prone to failure due to high memory load.

## 5a. Run Code

Start a new script in the project directory by entering
`nano run_bbduk_entropy.sh`. In the script, paste the following and then hit
Ctrl+S and Ctrl+X to save and exit. Note that the time is designed for only four
read files (two samples). Run the script using `sbatch run_bbduk_entropy.sh`.

If a copy of the removed sequences is desired, saveit in fastq.gz format using
the `outm` parameter in the bbduk command.
...
# Make the output directories
mkdir bbduk_out/entropy

# entropy=0.01 will remove monomeric repeats
unset JAVA_TOOL_OPTIONS
for file in sequences/*_1.fastq.gz; do
  sample=$(basename "$file" "_1.fastq.gz")  # Extracts the sample name
  bbduk.sh \
    in=hocort_out/"${sample}_1.fastq.gz" \
    in2=hocort_out/"${sample}_2.fastq.gz" \
    out=bbduk_out/entropy/"${sample}_1.fastq.gz" \
    out2=bbduk_out/entropy/"${sample}_2.fastq.gz" \
    entropy=0.01 \
    -Xmx20g
done
```
