...
bbduk, can perform a number of quality filtering steps. We will use it to do
three things:
...
3.  (Done later:) Remove any low-entropy reads (i.e. repetitive sequences)
...
The following code will remove adapters, contaminants, and quality trim/filter
reads, but will not remove monomeric repeats (this will be performed later).
...
for file in sequences/*_1.fastq.gz; do
  ...
  bbduk.sh \
    ...
    # Use almost all the allocated memory
    -Xmx45g
done
```
