---
title: Trimming Adapters with BBDuk 
output:
  html_document:
    toc: true     
    toc_depth: 3     
...
bbduk, can automatically find and remove a [range of common adapter sequences
and other similar
...
# Determine bbduk compatibility with your data
...
bbduk module. While bbduk is available as a module on Compute Canada, the
...
trimmed as expected, the output fastq.gz files can be used for the next step in
the pipeline.
...
other contaminant reads should be removed. For gut microbiomes, this is usually
reads mapping to host DNA.

[Step 2c: Remove human reads with
